...
state_names = 'null model state 1, null model state 2, null model state 3';
...
for n = 1 : length(subjectsBrainstormDB)
    ...
    acqBehaviouralPattern = fullfile(subjectsBehaviouralData(n).folder, subjectsBehaviouralData(n).name, 'null_model_state_timestamps*.csv');
    ...
    for acq = 1 : length(acqBehaviouralData)
        ...
        statesPattern = fullfile(subjectsBrainstormDB(n).folder, subjectsBrainstormDB(n).name, 'null_model_state_*');
